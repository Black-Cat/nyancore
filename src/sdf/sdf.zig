...
pub const EnterCommandFn = *const fn (ctxt: *IterationContext, iter: usize, mat_offset: usize, buffer: *[]u8) []const u8;
pub const ExitCommandFn = *const fn (ctxt: *IterationContext, iter: usize, buffer: *[]u8) []const u8;
pub const AppendMatCheckFn = *const fn (ctxt: *IterationContext, exit_command: []const u8, buffer: *[]u8, mat_offset: usize, alloc: std.mem.Allocator) []const u8;

pub const SphereBoundFn = *const fn (buffer: *[]u8, bound: *m.sphereBound, children: []m.sphereBound) void;
...
fn collectToKV(comptime collection: []const SdfInfo) []SdfKV {
    ...
    for (collection, kvs) |*sdf_info, *kvsi|
        kvsi.* = .{ .@"0" = sdf_info.name, .@"1" = sdf_info };
    return kvs[0..];
}
