...
pub const PipelineLayout = struct {
    ...
    pub fn create(descriptors: []const vk.DescriptorSetLayout, push_constants: []const vk.PushConstantRange) PipelineLayout {
        const create_info: vk.PipelineLayoutCreateInfo = .{
            .set_layout_count = @intCast(descriptors.len),
            .p_set_layouts = if (descriptors.len > 0) @ptrCast(descriptors.ptr) else undefined,
            .push_constant_range_count = @intCast(push_constants.len),
            .p_push_constant_ranges = if (push_constants.len > 0) @ptrCast(push_constants.ptr) else undefined,
            .flags = .{},
        };
